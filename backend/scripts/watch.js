...
async function runHook(hook, options, logger) {
  ...
  if (code !== 0) {
    throw new Error(`Hook '${hook.exec}' exited with code ${code}. Look at its logs for more information.`);
  }
...
/**
 * Creates a shutdown hook for the given child process. The returned hook as a `stop` function
 * that can be used to stop the process manually.
 */
